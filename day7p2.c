...
{
  int i = 0, highestMostCommonCard = -1, highestMostCommonCardIndex = -1;
  ...
  for (i = 0; i < CARDNUMBER; i++)
  {
    ...
    if (aux->cardMatchingCount[i] > highestMostCommonCard)
    {
      highestMostCommonCard = aux->cardMatchingCount[i];
      highestMostCommonCardIndex = i;
    }
  }

  aux->cardMatchingCount[highestMostCommonCardIndex] += aux->cardMatchingCount[3];
  aux->cardMatchingCount[3] = 0; //take jokers out
